...
\begin{frame}
  ...
  Quantifying Loss in Binned Scatterplots
    \begin{itemize}
...
\begin{center}
\large Loss and Binned Scatterplots\\
\normalsize Chapter 4
\end{center}
...
How do we discretize the frequency shading?
  \begin{itemize}
    ...
    \item Use one dimensional binning algorithms on bin counts, $c_\ell$
...
L^{\log F} = \sum_{\ell \in \mathscr{L}^\ast} (\log(c_\ell) - b_c(\log(c_\ell)))^2
\end{eqnarray}
...
3 Main Choices
  \begin{itemize}
    \item Number of frequency groups (k)
...
\begin{frame}
    \frametitle{Number of Frequency Groups}
...
\begin{frame}
    \frametitle{Frequency Loss Simulation}
...
\begin{frame}
    \frametitle{Log Frequency Loss Simulation}
...
p1 <- qplot(binxs, binys, geom="tile", fill=binfreq, data=binout1[[1]],
  ...
  theme_bw() + theme(legend.position="bottom",aspect.ratio=1,
                     panel.grid= element_blank(),
                     plot.margin = unit(c(.1,.1,.1,.1),"cm") )
p2 <- qplot(binfreq, geom="histogram", data=binout1[[1]], main="Histogram of Raw Bin Counts")+
  theme_bw() + xlab("Bin Frequency") + ylab("Number of Bins")
grid.arrange(p1,p2, nrow=1)
@  

\center Removes ability to see differences in low density bins
...
Transforming bin counts is one way to reduce visual impact of high density bins
