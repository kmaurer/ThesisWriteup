...
Why use binned scatterplots?
  \begin{itemize}
    \item Cannot effectively display large data with the traditional scatterplot
    \item Make information 'lost in translation' from visually rendering and reading the bivariate relationship explicit
  \end{itemize} 
...
Example Data from Baseball   
  \begin{itemize}
    ...
    \item Goal: Visualize relationship between games played and strike outs
  \end{itemize}
...
Plotting Binned Aggregations
  \begin{itemize}
    \item Sunflower Plots (Cleveland and McGill, 1984)
    \item Bubble Plots (Playfair, 1786)
    \item Binned Scatterplots (Unwin, Theus \& Hofmann, 2006)
  \end{itemize}
...
\begin{frame}
    \frametitle{Construction of Binned Scatterplots}

Reduce binned data triples:
  \begin{itemize}
    \item $(x^\ast_\ell, y^\ast_\ell, c_\ell)$ for all bins $\ell \in \{1, \dots, \mathscr{L} \}$
    \item $(x^\ast_\ell,y^\ast_\ell)$ = center of bin $\ell$
    \item $c_\ell$ = frequency count for bin $\ell$
    \item Triples are \textit{sufficient} for creating binned scatterplot
  \end{itemize}

\vspace{.1in}

Binning functions uniquely map data pairs to bin centers
  \begin{itemize}
    \item $b(.) : (x_i,y_i) \rightarrow (x^\ast_\ell,y^\ast_\ell)$ 
    \item Where $(x_i,y_i)$ are data pairs for observations $i \in \{1,\dots,n\}$
  \end{itemize}
...
\begin{frame}
    \frametitle{Rectangular Binning}

Rectangular binning functions bin each dimension univariately
  \begin{itemize}
    \item $b(x_i,y_i) = (b_X(x_i),b_Y(y_i))$
    \item Forms rectangular binned grid
    \item Computationally efficient (Wickham, 2013)
    \item Can generalize to higher dimension easily
  \end{itemize}
...
$J$ adjacent intervals covering range of the data. (Scott, 1979)

\vspace{.1in}

Parameters must be specified such that: $\beta_0 \le x_{(1)} \le x_{(n)} \le \beta_J$
where $x_{(1)}$ and $x_{(n)}$ are the minimum and maximum data values
...
\begin{frame}
    \frametitle{Standard Rectangular Binning}

Standard Rectangular Binning
  \begin{itemize}
    \item Use equally spaced intervals
    \item Bin width $= \omega_X$
    \item Bin origin $= \beta_0 \le x_{(1)}$
    \item $ \{\beta_j \text{ }|\text{ } \beta_j = \beta_{j-1} + \omega_X\}$ where $j \in \{1,\dots,J\}$
...
\begin{tabularx}{.7\textwidth}{rr} 
  $b_X(x)$ & \hspace{.4cm} $b_Y(y)$  \\
  \hline
  -5 & -5 \\ 
  -5 & -5 \\ 
  -5 & -5 \\ 
...
\begin{tabularx}{.9\textwidth}{rrr}

 \hspace{.2cm} $x^\ast$   &   \hspace{.2cm} $y^\ast$ & \hspace{.4cm}c \\ 
 \hline
-5 & -5 & 5 \\ 
  -5 & 5 & 2 \\ 
  5 & -5 & 3 \\ 
...
\vfil
\end{minipage}
\caption{\label{DataReductionExample}Original, Binned and Reduced Binned Data Tables, with data storage sizes.  Binned using standard rectangular approach with binning origin at (-10,-10) and bin widths $\omega_x = \omega_y = 10$. }
\end{table}
\normalsize
\end{frame}
...
\begin{frame}
    \frametitle{Random Rectangular Binning}

Random Rectangular Binning
  \begin{itemize}
    \item Define set of bin centers: $\{x_j^\ast \text{ }|\text{ } x_j^\ast > x_{j-1}^\ast \}$ for $j \in \{1,\dots,J\}$
    \item Assign points at random to surrounding bin centers
    \item Probabilities inversely proportional to distance from bin centers
  \end{itemize}
  
\vspace{.25in}
  
For $x_i$ such that $x^\ast_{j} \le x_i \le x^\ast_{j+1}$,
...
Slight issue with spatial loss for random binning
    \begin{itemize}
        ...
        \item Find equivalent assignment with minimum total spatial loss
...
Data pairs then rounded to nearest even integer to induce a 2X2 unit \textit{data resolution}
...
\begin{frame}
  ...
  \center Avoid binning origins with large overhangs outside data boundaries
...
A natural binning origin is ($x_{(1)}$, $y_{(1)}$)
  \begin{itemize}
    \item where $x_{(1)}$ and $y_{(1)}$ are the minimum data values in each dimension
    \item If data resolution is coarse, points not well centered within bins
  \end{itemize} 
  
A better default based on the data resolution
  \begin{itemize}
...
\begin{frame}
    \frametitle{Practical Takeaways for Spatial Binning}
...
How do we discretize the frequency shading?
  \begin{itemize}
    \item Create data triples $(x^\ast_\ell, y^\ast_\ell, c_\ell)$ for bins $\ell \in \{1, \dots, \mathscr{L} \}$
    \item Apply a univariate binning function to raw frequencies, $c_\ell$
    \item Shade ordinally based on binned frequencies, $b_C(c_\ell)$
...
Standard Frequency Binning
  \begin{itemize}
    \item Create k frequency groups using equal frequency bin widths
    \item Same uncertainty for each shade
  \end{itemize}  
\pause
...
Quantile Frequency Binning
  \begin{itemize}
    \item Create k frequency groups with empirical quantiles of raw frequencies
...
Frequency Loss
  \begin{itemize}
    \item Raw frequencies, $c_\ell$, for spatial bins $\ell \in \{1, \dots, \mathscr{L} \}$\\
...
Typical human perception $\Rightarrow$ k $\le 7$ (Healy and Enns, 1999)
...
labels = paste(c("[1,3]","(3,17]","(17,107]","(107,510]","(510,3150]"), "\n", c("1st Pentile","2nd Pentile","3rd Pentile","4th Pentile","5th Pentile"), sep="")
...
% Frequency Binning
%   \begin{itemize}
%     \item May be used to make frequency loss explicit
%     \item Use 4 to 7 frequency bins
%     \item Quantile frequency binning provides alternative display for skewed counts
%   \end{itemize}  
% 
% \end{frame}
...
\begin{frame}
  ...
  Used within course activities for \textit{Stat 104: Intro to Statistics}
...
Cronbach's $\alpha$ measures internal consistency within an item set
%
\begin{equation} \label{eq:alpha}
\alpha = K/(K-1) \cdot \left(1 - \sum_{i=1}^K \V{Y_i}  /  \V{\sum_{j=1}^K Y_j}\right)
\end{equation}
\begin{itemize}
  \item $Y_j$ denote the items $j \in \{1,\dots,K\}$ in the set
  \item Note: K=4 for sets in this survey
\end{itemize}
...
\begin{frame}
    \frametitle{Future Work for Thesis Components} 
Literature Review
  \begin{itemize}
    \item Section 2 to be submitted to JSE as review of technology in statistics education
  \end{itemize}
  
Statistical Inference Curriculum Comparison Study
  \begin{itemize}
    \item Accepted pending revision by TISE
    \item Awaiting results in second round of reviews
  \end{itemize}
    
Shiny Database Sampler
  \begin{itemize}
    \item Move application to more reliable R Server
    \item Manuscript to be submitted to TISE or SERJ
  \end{itemize}

Loss in Binned Scatterplots
  \begin{itemize}
    \item Manuscript to be submitted to JCGS
  \end{itemize}
